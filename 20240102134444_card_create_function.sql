...
CREATE FUNCTION public.create_subject_lesson_cards(_subject_id int8,_user_id uuid, _subject_name text, _current_level text, _lesson_title text, _lesson_description text, _cards jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$DECLARE
    _subject_id int8;
...
BEGIN
    ...
    RETURN json_build_object('lesson_id', _lesson_id, 'subject_id', _subject_id);
